...
class GlowDemo(ShowBase):
    ...
    def __init__(self):
        ...
        bk_text = "<Enter command>"
        #global textObject
        textObject = OnscreenText(text = bk_text, pos = (0.95,-0.95), scale = 0.07, fg=(1.0,0.5,0.5, 1.0), align=TextNode.ACenter, mayChange=1)
        textObject.setFont(fontToUse)
        #add button
        
        #b = DirectEntry(text = "" ,width=base.win.getXSize(), scale=0.05,command=setText(),
        #initialText="Type Something", numLines = 2,focus=1,focusInCommand=clearText(), entryFont=fontToUse, text_fg=(1.0,1.0,1.0,1.0), frameColor=(0.0,0.0,0.0,1.0))
        #add button
